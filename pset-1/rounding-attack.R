...
true.frac = matrix(nrow = num_roundings, ncol = num_samples)
#true0.frac = matrix(nrow = num_roundings, ncol = num_samples)
#vector of values we round to
round_to = vector('numeric', num_roundings)
for (i in 1:num_roundings) {
  round_to[i] = i
}
for (j in 1:num_roundings) {
  for (k in 1:num_samples) {
    which_round = round_to[j]
...
dev.copy2pdf(file="roundingResults.pdf")
