...
true.frac = matrix(nrow = num_subsamples, ncol = num_samples)
#true0.frac = matrix(nrow = num_subsamples, ncol = num_samples)
#vector of values we round to
subsampleSizes = vector('numeric', num_subsamples)
for (i in 1:num_subsamples) {
  subsampleSizes[i] = i*n/num_subsamples
}
for (j in 1:num_subsamples) {
  for (k in 1:num_samples) {
    s = subsampleSizes[j]
    subsampleIndex = sample(1:length(sensitiveData), s)
    history <- matrix(NA, nrow=k.trials, ncol=n+1)            # a matrix to store results in
    
    for(i in 1:k.trials){
      res <- query(n=q.size, data=sensitiveData, subsample = subsampleIndex)
...
num_filling = 0
avg.frac = vector('numeric', num_subsamples + num_filling)
count = 0
for (j in 1:num_subsamples) {
...
#subsampleSizes = sort(c(subsampleSizes, inserted_values))

plot(subsampleSizes, avg.frac, xlab = "subsample size", ylab = "fraction correct", ylim = c(0,1), col = "red")
